...
const ArtCard: React.FC<ArtCardProps> = ({ artist }) => {
  const { name, location, artType } = artist;
    ...
    <div className="flex-1">
      <div className="w-full h-full flex flex-col justify-between rounded border hover:shadow-lg">
        <div>
          ...
          <p className="p-4 pb-0 text-xl font-bold text-gray-900 line-clamp-1"> By {name} </p>
          <p className="px-4 pb-0 text-lg font-bold text-gray-600 line-clamp-1"> {location} </p>
          <div className="w-full flex justify-between px-4">
            <div className="flex items-center gap-2">
              <Rating rating={5} />
              (5)
            </div>
            <div className="rounded-full text-white bg-green-700 px-3 py-2"> {artType} </div>
          </div>
        </div>
        <div className="w-full p-4">
          <div className="w-full">
            <Link href={`/request-quote/${name.replace(' ', '')}`}>
